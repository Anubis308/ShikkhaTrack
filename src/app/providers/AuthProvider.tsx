...
import { fetchSessionClaims, loginWithPassword as signInWithPassword, logout as endSession, onSessionExpired, signupAccount, startLogin } from "../../lib/blocks/auth";
...
type AuthContextValue = {
  ...
  signup: (input: { email: string; firstName: string; lastName: string }) => Promise<{ signedIn: boolean }>;
  status: AuthStatus;
};
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signup = useCallback(async (input: { email: string; firstName: string; lastName: string }) => {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    signup,
    status
  }), [claims, login, loginWithPassword, logout, refresh, signup, status]);
